...
func TestTrialMiddleware_writesBlocked(t *testing.T) {
	...
	cases := []struct {
		...
		{"POST", "/api/counters"},
		{"PUT", "/api/counters/123"},
		{"DELETE", "/api/counters/123"},
		{"PUT", "/api/extras/counters/123"},
...
func TestTrialMiddleware_proTierBypassesGate(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/counters", bytes.NewReader(body))
	w := httptest.NewRecorder()
	srv.ServeHTTP(w, req)
	if w.Code == http.StatusPaymentRequired {
...
// newTestDB is a local helper since tally has no existing test file
// that defines one.
func newTestDB(t *testing.T) *store.DB {
	t.Helper()
	dir, err := os.MkdirTemp("", "tally-trial-test-*")
	if err != nil {
