...
async function load(){var r=await fetch(A+'/counters').then(function(r){return r.json()});items=r.counters||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.namespace||'').toLowerCase().includes(q)||(i.description||'').toLowerCase().includes(q)||(i.updated_at||'').toLowerCase().includes(q)});
...
if(i.namespace)h+='<div class="item-sub">'+esc(i.namespace)+'</div>';
...
if(i.updated_at)h+='<span>'+esc(i.updated_at)+'</span>';
if(i.counters)h+='<span>'+esc(i.counters)+'</span>';
if(i.namespaces)h+='<span>'+esc(i.namespaces)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/counters/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",namespace:"",description:"",updated_at:"",counters:"",namespaces:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' TALLY</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Namespace</label><input id="f-namespace" value="'+esc(i.namespace)+'"></div>';
h+='<div class="fr"><label>Description</label><textarea id="f-description" rows="3">'+esc(i.description)+'</textarea></div>';
h+='<div class="fr"><label>Updated At</label><input id="f-updated-at" value="'+esc(i.updated_at)+'"></div>';
h+='<div class="fr"><label>Counters</label><input id="f-counters" value="'+esc(i.counters)+'"></div>';
h+='<div class="fr"><label>Namespaces</label><input id="f-namespaces" value="'+esc(i.namespaces)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),namespace:document.getElementById('f-namespace').value.trim(),description:document.getElementById('f-description').value.trim(),updated_at:document.getElementById('f-updated-at').value.trim(),counters:document.getElementById('f-counters').value.trim(),namespaces:document.getElementById('f-namespaces').value.trim()};
if(editId){await fetch(A+'/counters/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/counters',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
