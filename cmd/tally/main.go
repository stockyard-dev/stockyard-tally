...
import (
	...
	"github.com/stockyard-dev/stockyard-tally/internal/license"
	"github.com/stockyard-dev/stockyard-tally/internal/server"
	"github.com/stockyard-dev/stockyard-tally/internal/store"
)

func main() {
	port := getEnv("PORT", "9110")
	dataDir := getEnv("DATA_DIR", "./data")
	licenseKey := os.Getenv("TALLY_LICENSE_KEY")
	...
	log.Printf("Stockyard Tally listening on :%s (tier: %s)", port, tier)
